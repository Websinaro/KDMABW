...
logger = logging.getLogger("weather_route")
...
async def get_weather(
	...
	# fetch_weather already retries transient failures and raises a clean
	# HTTPException(503) if Open-Meteo is genuinely unreachable - let that
	# propagate as-is.
	...
	try:
		...
		# Open-Meteo's response shape changed or a field was missing - this
		# used to crash as a bare, unexplained 500. Log the real cause and
		# surface a clean error instead.
		logger.error("Unexpected Open-Meteo response shape for lat=%s lon=%s: %s", lat, lon, exc)
		raise HTTPException(
			status_code=503,
